...
try {
  ...
  const oldInit = `const [activities, setActivities] = useState([
  ...
  const newInit = `const [activities, setActivities] = useState(() => {
        const saved = getStorageItem("global_activities");
  ...
  // Change hasUnreadActivities
  const oldUnreadInit = `const [hasUnreadActivities, setHasUnreadActivities] = useState(false);`;
  const newUnreadInit = `const [hasUnreadActivities, setHasUnreadActivities] = useState(() => {
        return getStorageItem("hasUnreadActivities") || false;
      });`;
  code = code.replace(oldUnreadInit, newUnreadInit);

  // Add useEffect to persist them
  ...
  const newUseEffect = `      useEffect(() => {
      ...
      useEffect(() => {
        setStorageItem("global_activities", activities);
      }, [activities]);

      useEffect(() => {
        setStorageItem("hasUnreadActivities", hasUnreadActivities);
      }, [hasUnreadActivities]);`;
  code = code.replace(oldUseEffect, newUseEffect);

  fs.writeFileSync('src/App.jsx', code, 'utf8');
  console.log("Persisted activities state.");
} catch(e) {
  console.error(e);
}
