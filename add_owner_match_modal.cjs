    ...
    function OwnerCreateMatchModal({ currentUser, venues, onClose, onSubmit }) {
      ...
      const handleFormSubmit = (e) => {
        ...
        onSubmit({
          ...
          district: "Thủ Đức", // Default for now or from myVenue
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md animate-fade-in px-4">
          ...
          <div className="w-full max-w-md bg-appDark-bg border border-appDark-border rounded-2xl p-5 space-y-4 relative z-10 shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar">
            ...
            <div className="flex justify-between items-center border-b border-appDark-border pb-3">
              ...
              <button onClick={onClose} className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white">✕</button>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <input type="text" required value={teamName} onChange={e => setTeamName(e.target.value)} placeholder="VD: FC Gà Mờ" className="w-full text-xs font-semibold bg-appDark-deep border border-appDark-border rounded-xl px-3 py-2 text-white focus:border-amber-500" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-slate-400">Trình Độ Yêu Cầu</label>
                  <select value={level} onChange={e => setLevel(e.target.value)} className="w-full text-xs font-semibold bg-appDark-deep border border-appDark-border rounded-xl px-3 py-2 text-white focus:border-amber-500">
...
const insertMarker = `    function MatchSuggestionModal({ match, matches, onClose }) {`;

code = code.replace(insertMarker, newModal + "\n" + insertMarker);

fs.writeFileSync('src/App.jsx', code);
console.log("Component OwnerCreateMatchModal injected");
