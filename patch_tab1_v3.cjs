                      ...
                      return (
                        <div className="space-y-4 animate-fade-in pb-10 mt-2">
                          ...
                          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
                            ...
                            <div className="flex flex-wrap items-center gap-3 md:gap-4 bg-appDark-card border border-appDark-border rounded-lg p-2.5 shadow-sm">
                              <div className={\`font-black text-[10px] md:text-xs uppercase flex items-center gap-1.5 \${systemStatusColor}\`}>
                                <span className="animate-pulse">\${systemStatusIcon}</span>
                                \${systemStatusText}
                              </div>
                              <div className="flex items-center gap-2 md:gap-3 text-[9px] md:text-[10px] font-medium text-slate-400 border-l border-appDark-border pl-2 md:pl-3">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <button onClick={() => setCurrentTab('admin_ql_san')} className="bg-appDark-card hover:bg-slate-800 border border-appDark-border hover:border-slate-500 rounded-lg px-2 py-1.5 text-[9px] md:text-[10px] font-bold text-slate-300 transition-all shadow flex items-center gap-1.5">
                                <span>➕</span> Duyệt chủ sân
                              </button>
                              <button onClick={() => setCurrentTab('admin_ql_keo')} className="bg-appDark-card hover:bg-slate-800 border border-appDark-border hover:border-slate-500 rounded-lg px-2 py-1.5 text-[9px] md:text-[10px] font-bold text-slate-300 transition-all shadow flex items-center gap-1.5">
                                <span>⚽</span> Xem kèo chờ đối
                              </button>
                              <button onClick={() => setCurrentTab('admin_ql_user')} className="bg-appDark-card hover:bg-slate-800 border border-appDark-border hover:border-slate-500 rounded-lg px-2 py-1.5 text-[9px] md:text-[10px] font-bold text-slate-300 transition-all shadow flex items-center gap-1.5">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div className="bg-appDark-deep border border-neon-green/30 rounded-xl p-4 shadow-md bg-gradient-to-br from-appDark-deep to-emerald-900/20">
                              ...
                              <span className="text-2xl md:text-3xl font-black text-neon-green">
                                {growth >= 0 ? '+' : ''}{growth}%
                              </span>
                            </div>
                          </div>
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {pendingVenues > 0 && (
                                  <button onClick={() => setCurrentTab('admin_ql_san')} className="text-left bg-appDark-card border border-red-500/20 hover:border-red-500/50 hover:bg-slate-800 rounded-lg p-2.5 transition-all">
                                    <span className="block text-[9px] md:text-[10px] text-slate-400 mb-1">Chờ duyệt sân</span>
                                    <span className="text-xs md:text-sm font-bold text-white">{pendingVenues} yêu cầu</span>
                                  </button>
                                )}
                                {pendingReports > 0 && (
                                  <button onClick={() => setCurrentTab('admin_ql_user')} className="text-left bg-appDark-card border border-red-500/20 hover:border-red-500/50 hover:bg-slate-800 rounded-lg p-2.5 transition-all">
                                    <span className="block text-[9px] md:text-[10px] text-slate-400 mb-1">Báo cáo vi phạm</span>
                                    <span className="text-xs md:text-sm font-bold text-white">{pendingReports} báo cáo</span>
                                  </button>
                                )}
                                {longWaitMatches > 0 && (
                                  <button onClick={() => setCurrentTab('admin_ql_keo')} className="text-left bg-appDark-card border border-amber-500/20 hover:border-amber-500/50 hover:bg-slate-800 rounded-lg p-2.5 transition-all">
                                    <span className="block text-[9px] md:text-[10px] text-slate-400 mb-1">Kèo chờ đối {'>'} 24h</span>
                                    <span className="text-xs md:text-sm font-bold text-white">{longWaitMatches} kèo</span>
                                  </button>
                                )}
                                {expiringSlots > 0 && (
                                  <button onClick={() => setCurrentTab('admin_ql_san')} className="text-left bg-appDark-card border border-amber-500/20 hover:border-amber-500/50 hover:bg-slate-800 rounded-lg p-2.5 transition-all">
                                    <span className="block text-[9px] md:text-[10px] text-slate-400 mb-1">Slot sắp hết hạn</span>
                                    <span className="text-xs md:text-sm font-bold text-white">{expiringSlots} slot</span>
                                  </button>
                                )}
                              </div>
                            </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="bg-appDark-deep border border-blue-500/30 rounded-xl p-3.5 shadow-sm">
                              <span className="text-[9px] md:text-[10px] font-bold text-blue-400 uppercase tracking-wider block truncate">🏟️ Sân Active</span>
                              <span className="text-lg md:text-xl font-black text-white mt-1.5 block">{activeVenues}</span>
                            </div>
                            <div className="bg-appDark-deep border border-blue-500/30 rounded-xl p-3.5 shadow-sm">
                              <span className="text-[9px] md:text-[10px] font-bold text-blue-400 uppercase tracking-wider block truncate">👥 Đội Active</span>
                            ...
                            <div className="bg-appDark-deep border border-neon-yellow/30 rounded-xl p-3.5 shadow-[0_0_10px_rgba(253,224,71,0.1)]">
                              <span className="text-[9px] md:text-[10px] font-bold text-neon-yellow uppercase tracking-wider block truncate">🟡 Kèo Đang Mở</span>
                            ...
                            <div className="bg-appDark-deep border border-appDark-border rounded-xl p-3.5 shadow-sm">
                              <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider block truncate">📍 Khu Vực Hot</span>
                              <span className="text-sm md:text-base font-black text-white mt-1.5 block truncate" title={topDistrictName}>{topDistrictName}</span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="bg-appDark-deep border border-neon-green/30 rounded-xl p-3.5 shadow-sm overflow-hidden">
                              <span className="text-[9px] md:text-[10px] font-bold text-neon-green uppercase tracking-wider block truncate">🔥 Fill Rate</span>
                              <span className="text-lg md:text-xl font-black text-white mt-1.5 block">{fillRate}%</span>
                            </div>
                            <div className="bg-appDark-deep border border-neon-green/30 rounded-xl p-3.5 shadow-sm overflow-hidden">
                              <span className="text-[9px] md:text-[10px] font-bold text-neon-green uppercase tracking-wider block truncate">🤝 Tỷ Lệ Chốt</span>
                            ...
                            <div className="bg-appDark-deep border border-red-500/30 rounded-xl p-3.5 shadow-[0_0_10px_rgba(239,68,68,0.1)] overflow-hidden">
                              <span className="text-[9px] md:text-[10px] font-bold text-red-400 uppercase tracking-wider block truncate">❌ Tỷ Lệ Hủy</span>
                            ...
                            <div className="bg-appDark-deep border border-amber-500/30 rounded-xl p-3.5 shadow-[0_0_10px_rgba(245,158,11,0.1)] overflow-hidden">
                              <span className="text-[9px] md:text-[10px] font-bold text-amber-500 uppercase tracking-wider block truncate">💰 Hoa Hồng</span>
...
console.log("Tab 1 refactored layout fixed successfully.");
